...
def extract_funding_state(root: etree._Element) -> ArticleFundingState:
    ...
    # AGENT INSTRUCTION: Legacy JATS extracts from //article-meta/contract-num and contract-sponsor.
    # Coalesce both into a standardized [{agency, grant_id}] structure.
    ...
    if contract_sponsors and hasattr(contract_sponsors, "__iter__"):
        agency_text = " ".join("".join(str(t) for t in s.itertext()).strip() for s in contract_sponsors).strip()

    if contract_nums and hasattr(contract_nums, "__iter__"):
        grant_id_text = " ".join("".join(str(t) for t in n.itertext()).strip() for n in contract_nums).strip()

    if agency_text or grant_id_text:
        funding.append(FundingEntityState(agency=agency_text, grant_id=grant_id_text))
