...
def mock_fsspec_filesystem() -> Generator[Tuple[MagicMock, MagicMock, MagicMock], None, None]:
    with patch("fsspec.filesystem") as mock_fs_factory:
        ...
        def side_effect(protocol: str, **kwargs: Any) -> MagicMock:
            if protocol == "s3":
                return mock_s3
            if protocol == "ftp":
                return mock_ftp
...
def test_s3_client_error_no_failover(
    source_manager: SourceManager,
    mock_fsspec_filesystem: Tuple[MagicMock, MagicMock, MagicMock],
) -> None:
...
def test_s3_connection_error_failover(
    source_manager: SourceManager,
    mock_fsspec_filesystem: Tuple[MagicMock, MagicMock, MagicMock],
) -> None:
    ...
    mock_s3.cat.side_effect = IOError("Connection Reset")
    ...
    mock_ftp.cat.assert_called_with("/pub/pmc/file3.xml")


def test_s3_intermittent_failure_resets_counter(
    source_manager: SourceManager,
    mock_fsspec_filesystem: Tuple[MagicMock, MagicMock, MagicMock],
) -> None:
    """Verify success resets the error counter."""
    _, mock_s3, _ = mock_fsspec_filesystem
    mock_s3.cat.side_effect = [IOError("Error"), b"success"]
...
def test_s3_client_error_resets_counter(
    source_manager: SourceManager,
    mock_fsspec_filesystem: Tuple[MagicMock, MagicMock, MagicMock],
) -> None:
    """Verify 404 (ClientError) resets the connection error counter."""
    _, mock_s3, _ = mock_fsspec_filesystem
    mock_s3.cat.side_effect = [IOError("ConnError"), FileNotFoundError("404")]
...
def test_ftp_usage_after_failover(
    source_manager: SourceManager,
    mock_fsspec_filesystem: Tuple[MagicMock, MagicMock, MagicMock],
) -> None:
